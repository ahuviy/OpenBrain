...
Two tiers. **Vocabulary and merges apply immediately** — both are reversible via `archived` +
`supersedes`, and what a merge collapsed comes back in `applied_items`, since nothing else records
...
`Bert Dohmen` into `Dohmen` (or the reverse — the spelling the brain already uses most wins, so the
fewest rows change). It reaches thoughts the watermark excludes, because the rows carrying an old
spelling are precisely the old ones. It never folds `markets` into `market-analysis`: those are
different subjects, not spellings, and that is a judgment call. They come back in `items` — each
...
**A consolidation fix only reaches thoughts the run looks at, and the watermark only moves forward.**
...
thought id and both sides of the rewrite.

The watermark stops short of any thought left in an unreviewed proposal, so a proposal that expires
or is lost can be regenerated by the next run rather than becoming unreachable. Those pairs are
re-judged on every run until the proposal is reviewed.
...
| `since` | string | Re-examine everything changed since this ISO timestamp instead of since the last run. `"1970-01-01"` is a full-corpus pass |
...
```json
{
  ...
  "applied_items": [
    { "kind": "merge", "sources": [ { "id": "e5f6…", "content": "…" }, { "id": "g7h8…", "content": "…" } ] }
  ],
  "candidates": 37,
  "clusters": 8,
...
`overlaps` names any thought appearing in more than one item, and which items would archive it. Items
are judged pairwise, so one thought can be obsolete in one and the survivor in another; the run drops
the item that would archive a thought another item depends on (counted as `skipped.proposal_conflict`),
and `overlaps` reports whatever legitimately remains. A contradiction item also carries a `caution`
when the thought it would archive is long and much bigger than the one superseding it — accepting
retires all of it, including unrelated material.
...
Also returns `pending_proposal` — `{ id, expires_at, item_count }` or `null`. Proposals expire after
`DREAM_PROPOSAL_TTL_HOURS` (default 72), and **expiry is not a reject**: a rejected proposal is a
decision, recorded as `applied` with everything in `rejected`, while an expired one is a decision
nobody made. Either way nothing is archived and the judgments are gone.
